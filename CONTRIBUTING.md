...
```bash
uv run python doostan.py
```

Against a custom database path:

```bash
uv run python doostan.py --db-path ~/addresses.db
```
...
1. Create `doost/plugins/<format>_plugin.py` implementing the
...
2. Import it in `doost/plugins/__init__.py`:

   ```python
   from doost.plugins.yaml_plugin import YAMLPlugin  # noqa
   ```
